 ...
 */
export function storeSigningKey(signingKey, keyId, persistent = false, expiresAt = null) {
    const val = JSON.stringify({ signingKey, keyId, expiresAt });
    if (persistent) {
...
// Guest keys are issued with a 10-min TTL server-side; refresh a little early so a
// request signed right at the boundary is not rejected.
const GUEST_KEY_TTL_MS = 9 * 60 * 1000;

/**
 * initGuestKey(appId)
 * No-ops if a usable key already exists. Fetches a 10-min guest signing key from /api/auth/guest-key.
 * Always stores in sessionStorage (guest keys never persist across browser close).
 * A stored guest key past its TTL is treated as absent and replaced.
 */
export async function initGuestKey(appId) {
    ...
    // no-store: never let the service worker or HTTP cache hand back a key issued earlier
    const res = await fetch(`/api/auth/guest-key?app=${encodeURIComponent(appId)}`, { cache: 'no-store' });
    if (!res.ok) throw new Error('Failed to initialise guest signing key');
    const { key_id, signing_key } = await res.json();
    storeSigningKey(signing_key, key_id, false, Date.now() + GUEST_KEY_TTL_MS);
}
